...
if __name__ == "__main__":
    ...
    # Espace massif en haut pour le diagramme (1.45x la valeur max)
    ...
    # 2. Flèche CPU Bound (Gauche)
    ax.annotate('', xy=(10000, max_y * 1.25), xytext=(l3_boundary, max_y * 1.25),
                arrowprops=dict(arrowstyle='<->', color='#2c3e50', lw=2.5))
    ax.text(28000, max_y * 1.27, "COMPUTE BOUND (L1/L2 CACHE)\nCompiler vectorizes ECS loop.\nCRG pays the Dispatch Tax.", 
            color='#2c3e50', ha='center', va='bottom', fontsize=10, fontweight='bold')

    # 3. Flèche Memory Bound (Droite)
    ax.annotate('', xy=(l3_boundary, max_y * 1.25), xytext=(10000000, max_y * 1.25),
                arrowprops=dict(arrowstyle='<->', color='#2980b9', lw=2.5))
    ax.text(800000, max_y * 1.27, "MEMORY BOUND (MAIN RAM)\nCPU starves for data. The Dispatch Tax is hidden by RAM latency.\nCRG Bandwidth converges with ECS.", 
            color='#2980b9', ha='center', va='bottom', fontsize=10, fontweight='bold')
    ...
    # Texte ECS (en bas à gauche)
    ax.text(0.02, 0.15, 
            "The Pure ECS Illusion:\nAn array of identical entities processed by a single function\nis incredibly fast in cache (gray line), but games require\npolymorphism, which breaks this ideal scenario.", 
            transform=ax.transAxes, fontsize=11, bbox=props, va='bottom', ha='left', zorder=5)
            
    # Texte CRG (au milieu à droite)
    ax.text(0.98, 0.55, 
            "The CRG Trade-off:\nCRG sacrifices some cache-level throughput to enable O(1) logic injection.\nOnce we hit the RAM limit, this overhead becomes negligible,\nproving that OOP flexibility is effectively 'free' at scale.", 
    ...
    plt.tight_layout()
    output_file = "crg_raw_performance_architect.png"
