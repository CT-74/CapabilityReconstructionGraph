...
class PPTXGenerator:
    ...
    def create_deck(self, slides_data):
        for data in slides_data:
            ...
            # Identify high-impact slides requiring the special tri-zone layout
            is_bench_slide = any(k in title_upper for k in ["BENCHMARK", "STRESS TEST", "RESULTS"])

            # --- SPECIAL LAYOUT: PERFORMANCE SHOWCASE (TABLE + VIDEO + QR) ---
            if is_bench_slide:
                # 1. Benchmark Table/Chart (Left)
        ...
        # Save output based on target language
        output_name = f"CRG_Talk_{self.lang}.pptx"
        self.prs.save(os.path.join(CURRENT_DIR, output_name))

if __name__ == "__main__":
    # 1. Core Asset Generation
    generate_simulator_qr()
    
    # Update benchmarks if script is available
    ...
    # 2. Markdown Parsing & Deck Generation
    try:
